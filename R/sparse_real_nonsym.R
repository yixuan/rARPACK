eigs.sparse_real_nonsym <- function(A, k, which = "LM", sigma = 0.0,
                               opts = list())
{
    n = nrow(A);
    if (n != ncol(A))
    ...
    res = .Call("sparse_real_nonsym", A, as.integer(n), as.integer(k),
...
setMethod("eigs", signature(A = "dgCMatrix",
          ...
          function(A, k, which, sigma, ...)
              eigs.sparse_real_nonsym(A, k, which, sigma, list(), ...));
##' @rdname eigs-methods
...
setMethod("eigs", signature(A = "dgCMatrix",
                            ...
                            opts = "missing"),
          function(A, k, which, ...)
              eigs.sparse_real_nonsym(A, k, which, 0.0, list(), ...));
##' @rdname eigs-methods
...
setMethod("eigs", signature(A = "dgCMatrix",
                            ...
                            opts = "missing"),
          function(A, k, which, ...)
              eigs.sparse_real_nonsym(A, k, "LM", 0.0, list(), ...));
